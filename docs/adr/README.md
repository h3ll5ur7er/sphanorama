...
| [0061](0061-the-half-pixel-convention-gap-is-measured-and-kept.md) | Keypoints reach `camera_model` half a pixel out, worth 0.0103° per pair — and correcting it makes every published median *worse* and fails the ORB bound, because the table is green partly by cancellation against the detectors' own localisation bias. Kept, measured, and the correction's blast radius named |
