...
| [0030](0030-the-spill-tier-is-resident-and-carries-its-own-index.md) | The OPFS spill tier has a fixed preferred name and a sibling index, so a reload can find the frames its session document names — its reload fallback **narrowed by [0063](0063-a-reload-waits-for-the-resident-spill-pair.md)**, which waits for the pair first |
...
| [0063](0063-a-reload-waits-for-the-resident-spill-pair.md) | A reload waits up to about three seconds — a busy worker was measured at two — for its previous worker to release the resident spill pair before taking a tier of its own, because falling back at once made the capture it came back for unresumable. Only a tab that held the pair last time waits, by a claim in `sessionStorage`: a second tab that waited took the pair from its sibling's reload |
