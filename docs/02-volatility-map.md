...
| V17 | **What this device's camera is known to be** — the lens kept between captures, how one more capture amends it, what it is keyed by | Device, the browser's camera identity, how estimates are combined | `utilities/kept_lens` for the amendment; V12's store for where it lives (ADR 0067) |
...
not, so a browser build — which gets the null engine (ADR 0052) — could have the half of registration
that is quaternions once a `Refine` lives outside the OpenCV engine; today it has none of it. `quaternion_average` sits under both it and
...
including why that tool implements the lens itself rather than calling `camera_model`; ADR 0059
holds which panorama, and why a committed one needs a record beside it.
