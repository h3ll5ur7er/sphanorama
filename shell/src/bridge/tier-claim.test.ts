...
describe("a tab's claim on the resident spill tier", () => {
  ...
  it('is simply not held where reaching the storage at all throws, as Chromium does when it is off', () => {
    // Measured: a sandboxed frame gets a SecurityError from the `sessionStorage` property itself.
    const unreachable = () => { throw new DOMException('denied', 'SecurityError'); };
    expect(tabClaim(unreachable).held).toBe(false);
    expect(() => tabClaim(unreachable).record(true)).not.toThrow();
  });
});
