...
| A Python test red on correct code, just after a sabotage sweep | Python reuses `__pycache__` when the source's size and whole-second mtime match. A same-size sabotage, restored within the second it was compiled, keeps running from the cache against the restored source. It happened: the gate failed on a sampler whose file was correct. Sweep with `PYTHONDONTWRITEBYTECODE=1` |
