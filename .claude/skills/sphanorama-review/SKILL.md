...
| **Docs and ADRs** | Did the change invalidate a sentence somewhere? `docs/06-roadmap.md`, the volatility map, the contract READMEs and the engineering skill all make claims the code has to keep. An ADR is required for everything on `sphanorama-engineering/SKILL.md`'s "Write an ADR" list — read it rather than this row, which used to restate it and fell two triggers behind — and its *consequences* section is where the costs it accepted belong |
