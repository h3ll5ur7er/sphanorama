...
class CaptureSessionRetakes : public CaptureSession {
 protected:
  ...
  std::unique_ptr<CaptureSessionManager> Rebuilt() {
    return std::make_unique<CaptureSessionManager>(planner, pose, newestFirst, preview, *camera,
                                                   *sensor, *store, *projects, clock);
  }

  std::unique_ptr<CaptureSessionManager> RebuiltForgetful() {
    return std::make_unique<CaptureSessionManager>(planner, pose, forgetful, preview, *camera,
                                                   *sensor, *store, *projects, clock);
  }
};
